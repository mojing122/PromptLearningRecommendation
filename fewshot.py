import tqdm
from openprompt.data_utils.text_classification_dataset import AgnewsProcessor, DBpediaProcessor, ImdbProcessor, AmazonProcessor , NewstitleProcessor, MovielensProcessor
...
class MIND(DataProcessor):

    def __init__(self):
        super().__init__()
        self.labels = ['won\'t', 'will']
    ...
    def get_examples(self, data_dir, split):
        ...
        with open(path, encoding='utf8') as f:
            ...
            for idx, row in enumerate(reader):
                ...
                example = InputExample(text_a=text_a, text_b=text_b, label=int(label))
...
if args.dataset == "agnews":
    ...
    dataset['train'] = DBpediaProcessor().get_train_examples("./datasets/TextClassification/dbpedia/")
    dataset['test'] = DBpediaProcessor().get_test_examples("./datasets/TextClassification/dbpedia/")
    ...
    dataset['train'] = AmazonProcessor().get_train_examples("./datasets/TextClassification/amazon/")
    dataset['test'] = AmazonProcessor().get_test_examples("./datasets/TextClassification/amazon/")
    ...
    dataset['train'] = MovielensProcessor().get_train_examples("./datasets/RecommendationSystem/movielens/")
    dataset['test'] = MovielensProcessor().get_test_examples("./datasets/RecommendationSystem/movielens/")
    class_labels =MovielensProcessor().get_labels()
    scriptsbase = "RecommendationSystem/movielens"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 128
    batch_s = 30
...
csv_writer.writerow([args.uid,args.rate_num,test_acc[0],test_acc[1],test_acc[2]])
f.close()
import os
